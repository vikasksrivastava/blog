...
<!-- code_chunk_output -->
		...
		* [To link the newly compiled NEDS](#to-link-the-newly-compiled-neds)
			* [Next Objective is to Add emulated devices to the NSO and perform some intial taks like Synchornize Config , Create Device Groups , and make sure all devices share the same config.](#next-objective-is-to-add-emulated-devices-to-the-nso-and-perform-some-intial-taks-like-synchornize-config-create-device-groups-and-make-sure-all-devices-share-the-same-config)
		* [Add Emulated Devices to the Network](#add-emulated-devices-to-the-network)
		* [Create device with Net Sim](#create-device-with-net-sim)
		* [Create Device Group](#create-device-group)
		* [Create Customer](#create-customer)
		* [Lets now create a Device template](#lets-now-create-a-device-template)
		* [Now Lets create a Service](#now-lets-create-a-service)
		* [Lets Start with creating an L2 VPN YANG template](#lets-start-with-creating-an-l2-vpn-yang-template)
		* [Now Lets create a Device Template based on the Actual CLI configuration](#now-lets-create-a-device-template-based-on-the-actual-cli-configuration)
		* [Assign Service Instances to Customers](#assign-service-instances-to-customers)
		* [Creating a Custom Auth Group for "Real Cisco Devices"](#creating-a-custom-auth-group-for-real-cisco-devices)
		* [You Can Install Custom Commands at `/ncs-run/scripts/command`](#you-can-install-custom-commands-at-ncs-runscriptscommand)
		* [NCS Troubleshooting commands](#ncs-troubleshooting-commands)
		* [Service Model Optimisation](#service-model-optimisation)
* [Start at Page 66](#start-at-page-66)
		* [Deploy your own blankent OSPF Service on Dynamips routers to make them part of an OSPF Area](#deploy-your-own-blankent-ospf-service-on-dynamips-routers-to-make-them-part-of-an-ospf-area)
		* [Makefile Training https://www.youtube.com/watch?v=Q1Lnp_Xx7z4](#makefile-training-httpswwwyoutubecomwatchvq1lnp_xx7z4)
		* [Create A Directory Structure of Where the YANG is Stored and Where is the Correcpoding  Device Template Cofiguration is stored .](#create-a-directory-structure-of-where-the-yang-is-stored-and-where-is-the-correcpoding-device-template-cofiguration-is-stored)
		* [Create a Hierarchy Diagram of How the NSO components integrate together for a high leve glane of the over all solution .](#create-a-hierarchy-diagram-of-how-the-nso-components-integrate-together-for-a-high-leve-glane-of-the-over-all-solution)
		* [Put a place holder to Start documenting vMS / NFV / CSP Solution product portfolio to learn them .](#put-a-place-holder-to-start-documenting-vms-nfv-csp-solution-product-portfolio-to-learn-them)
		* [https://www.ansible.com/networks-with-cisco-nso-ansible](#httpswwwansiblecomnetworks-with-cisco-nso-ansible)
		* [Supported NEDS](#supported-neds)
